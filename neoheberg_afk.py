...
1) 设置环境变量（二选一）:
   # 方式 A —— 把 COOKIE 导出成一行（推荐，在浏览器 F12→Application→Cookies 抄
   #           __Host-NH-Remember 和 __Host-NH 两个值），放到环境变量:
   export NH_REMEMBER="aQvZA...      <-- __Host-NH-Remember 的值"
   export NH_SESSION="A89G...        <-- __Host-NH 的值"

   # 方式 B —— 或直接给完整 Cookie 头字符串（连分号一起）：
...
4) systemd 常驻(可选) 见文件末尾注释。

cookie 过期(约1天)症状: 日志出现 "session 过期" → 重新导出 __Host-NH 更新环境变量重启即可。
───────────────────────────────────────────────────────────────────
"""
...
# ════════════════════════════════════════════════════════════════════
# 认证头(可以提供 Cookie 头字符串或拆分的两个 cookie 值)
# ════════════════════════════════════════════════════════════════════
def cookie_header() -> str:
    """返回可用的 Cookie 请求头。优先用整条 NH_COOKIE_HEADER，否则拼两要件。"""
    if NH_COOKIE_HEADER:
        return NH_COOKIE_HEADER
    parts = []
    if NH_REMEMBER:
        parts.append(f"__Host-NH-Remember={NH_REMEMBER}")
    if NH_SESSION:
        parts.append(f"__Host-NH={NH_SESSION}")
    if not parts:
        raise RuntimeError("请设置 NH_COOKIE_HEADER 或 NH_REMEMBER/NH_SESSION 环境变量")
    return "; ".join(parts)
...
# ════════════════════════════════════════════════════════════════════
def _get(url, **kw):
    headers = {"User-Agent": NH_UA, "Cookie": cookie_header()}
